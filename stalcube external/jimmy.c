...
void printAllOffsets() {
	printf("JVM_SEED_SYMBOLTABLE          = jvm + 0x%X = %p\n", JVM_SEED_SYMBOLTABLE, PTRMATH(jvm + JVM_SEED_SYMBOLTABLE));
	printf("systemCl          = jvm + 0x%X = %p\n", JVM_SYSTEMCL, PTRMATH(jvm + JVM_SYSTEMCL));
	printf("JVM_SYMBOLTABLE               = jvm + 0x%X = %p\n", JVM_SYMBOLTABLE, PTRMATH(jvm + JVM_SYMBOLTABLE));
	printf("JVM_SYSTEMDICTIONARY          = jvm + 0x%X = %p\n", JVM_SYSTEMDICTIONARY, PTRMATH(jvm + JVM_SYSTEMDICTIONARY));
	printf("JVM_SYSTEMCL                  = jvm + 0x%X = %p\n", JVM_SYSTEMCL, PTRMATH(jvm + JVM_SYSTEMCL));

	printf("JVM_USE_COMPRESSED_CLASS_PTRS = jvm + 0x%X = %p\n", JVM_USE_COMPRESSED_CLASS_POINTERS, PTRMATH(jvm + JVM_USE_COMPRESSED_CLASS_POINTERS));
	printf("JVM_CLS_PTRS_BASE             = jvm + 0x%X = %p\n", JVM_COMPRESSED_CLASS_POINTERS_BASE, PTRMATH(jvm + JVM_COMPRESSED_CLASS_POINTERS_BASE));
	printf("JVM_CLS_PTRS_SHIFT            = jvm + 0x%X = %p\n", JVM_COMPRESSED_CLASS_POINTERS_SHIFT, PTRMATH(jvm + JVM_COMPRESSED_CLASS_POINTERS_SHIFT));

	printf("JVM_USE_COMPRESSED_OOPS       = jvm + 0x%X = %p\n", JVM_USE_COMPRESSED_OOPS, PTRMATH(jvm + JVM_USE_COMPRESSED_OOPS));
	printf("JVM_OOPS_BASE                 = jvm + 0x%X = %p\n", JVM_COMPRESSED_OOPS_BASE, PTRMATH(jvm + JVM_COMPRESSED_OOPS_BASE));
	printf("JVM_OOPS_SHIFT                = jvm + 0x%X = %p\n", JVM_COMPRESSED_OOPS_SHIFT, PTRMATH(jvm + JVM_COMPRESSED_OOPS_SHIFT));
}
...
system_loader()
{
	...
	printf("systemcl %p  -  jvm+ctl %p - jvm %p\n", systemCl, PTRMATH(jvm + JVM_SYSTEMCL), jvm);
	return systemCl;
}
...
jmy_getobjectclass(Jobj oop)
{
	...
	if (flag) {
		uint32_t compressed_klass;
		readmem(&compressed_klass, PTRMATH(oop + JVM_OOP_KLASS), sizeof(compressed_klass));
		return klass = decodeklass(compressed_klass);
	}
		
	readmem(&klass, PTRMATH(oop + JVM_OOP_KLASS), sizeof(klass));
	return klass;
}
